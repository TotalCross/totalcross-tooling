...
export async function analyzeLegacyProject(context: vscode.ExtensionContext, folder?: vscode.WorkspaceFolder): Promise<void> {
    ...
    try {
        const command = companionCommand(context.extensionPath, 'java', ['analyze', '--project', selected.uri.fsPath, '--plan', plan]);
        ...
        const applied = await runConversion(companionCommand(context.extensionPath, 'java', ['apply', '--plan', plan]), selected.uri.fsPath);
        ...
        try {
            await runConversion(companionCommand(context.extensionPath, 'java', ['validate', '--project', selected.uri.fsPath]), selected.uri.fsPath);
            vscode.window.showInformationMessage('TotalCross project conversion completed and validated.');
        } catch (validationError) {
            await runConversion(companionCommand(context.extensionPath, 'java', ['rollback', '--journal', event.journal]), selected.uri.fsPath);
