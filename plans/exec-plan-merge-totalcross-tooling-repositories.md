...
- [ ] Rewrite a disposable VS Code plugin clone into `vscode-plugin/`, preserve commit metadata and topology, namespace tags, and save its old-to-new commit map.
...
- Decision: Use the exact final subdirectories `maven-plugin/`, `vscode-plugin/`, and `gradle-plugin/` in that import order.
...
- Decision: Namespace imported tags as `maven-plugin-*`, `vscode-plugin-*`, and `gradle-plugin-*`.
...
- update build paths and CI working directories for `vscode-plugin/`;
...
- `vscode-plugin`: run when `vscode-plugin/**` or shared governance affecting it changes;
...
Validate path history and blame on representative files from each project:
    ...
    git log --follow -- vscode-plugin/src/extension.ts
    git log --follow -- gradle-plugin/path/to/file

    git blame maven-plugin/path/to/file
    git blame vscode-plugin/src/extension.ts
...
Use the equivalent `vscode-plugin/` and `gradle-plugin/` links in the other repositories.
...
- Source is located under `maven-plugin/`, `vscode-plugin/`, and `gradle-plugin/`.
...
The validator must expose a command-line interface equivalent to:
    ...
    python3 tools/check-license-headers.py
    python3 tools/check-license-headers.py --project vscode-plugin
