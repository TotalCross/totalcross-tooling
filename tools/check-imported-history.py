...
PROJECTS = {"maven-plugin": "master", "vscode-extension": "master", "gradle-plugin": "main"}
...
def audit(source: Path, target: Path, project: str) -> list[str]:
    ...
    source_commits = git(source / f"totalcross-{project.replace('-plugin', '')}-plugin.git" if project != "vscode-extension" else source / "totalcross-vscode-plugin.git", "rev-list", branch).decode().split()
    errors = []
